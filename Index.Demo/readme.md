...
Примечание 1: Запросам с постфиксом `*war` свойственна та же проблема полного сканирования индекса.
`Warning` на такие запросы не был реализован.

Примечание 2: Нечёткий запрос, например `lwanowr~0.2` с низким порогом сходства обрабатывается ещё 
хуже - выполнение такого запроса может даже зависнуть вне зависимости от малости индекса.
`Warning` на такие запросы не был реализован.
...
Warning на суффиксные запросы и на нечёткие (Fuzzy) запросы с низким порогом сходства
-------------------------------------------------------------------------------------

см. примечания к разделу "Обработка запроса отрицания на языке Lucene"
